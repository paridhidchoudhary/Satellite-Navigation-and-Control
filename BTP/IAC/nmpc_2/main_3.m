...
th_mag = zeros(8,size(U_opt,2));         % 8×N
for k = 1:8
    fk = U_opt(3*(k-1)+1:3*k,:);
    th_mag(k,:) = vecnorm(fk,2,1);
end

figure
plot(t_control, th_mag.')               % the transpose gives 8 coloured lines
grid on; xlabel('Time [s]'); ylabel('Force [N]');
legend(arrayfun(@(k) sprintf('f_%d',k),1:8,'UniformOutput',false));
title('Thrust magnitudes – all eight jets');
